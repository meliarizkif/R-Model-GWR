#Imputasi
library(VIM)
impute1 <- kNN(X2019,variable = c("PAD","Belanja Daerah"),k=7)
summary(impute1)
actuals <-X2019$PAD
predicteds <- impute1[is.na(X2019$PAD), "PAD"]
mape(actuals, predicteds)
rmse(actuals,predicteds)
my.statistics(Actual=X2019$PAD,Predicted=impute1)
impute2 <-kNN(X2019)
summary(impute2)

write.xlsx(impute1, file = "imputasi2019.xlsx",sheetName = "2019", append = FALSE)
